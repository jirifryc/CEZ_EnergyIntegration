...
HA Energy Dashboard), yesterday's interval breakdown, and latest power
from 15-min interval data.

PND interval data becomes available for the previous day after ~1 AM,
so the "interval" sensors always reflect yesterday's data.
"""
...
async def async_setup_entry(hass: HomeAssistant, entry, async_add_entities):
    ...
    entities = [
        ...
        CezYesterdayEnergySensor(hub, eid, "nt", "ČEZ Včerejší NT", f"{eid}_yesterday_nt"),
        CezYesterdayEnergySensor(hub, eid, "vt", "ČEZ Včerejší VT", f"{eid}_yesterday_vt"),
        CezYesterdayEnergySensor(hub, eid, "total", "ČEZ Včerejší celkem", f"{eid}_yesterday_total"),
        CezLastPowerSensor(hub, eid),
    ]
...
class CezCumulativeEnergySensor(CoordinatorEntity, SensorEntity):
    ...
    Shows the authoritative cumulative reading from the daily endpoint.
    Updated once a day when the daily coordinator refreshes.
    """
...
class CezYesterdayEnergySensor(CoordinatorEntity, SensorEntity):
    """Yesterday's NT/VT/Total consumption from 15-min interval data."""
...
class CezLastPowerSensor(CoordinatorEntity, SensorEntity):
    """Latest known 15-minute average power (from yesterday's last interval)."""
    ...
    def __init__(self, hub: CezEnergyHub, electrometer_id: str) -> None:
        super().__init__(hub.realtime_coordinator)
        self._hub = hub
        self._attr_name = "ČEZ Poslední výkon"
        self._attr_unique_id = f"{electrometer_id}_last_power"
        self._attr_device_info = hub.device_info
